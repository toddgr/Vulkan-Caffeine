...
Init13DescriptorSetPool()
{
	...
	VkDescriptorPoolSize				vdps[NUMDS];
...
///		vsi.signalSemaphoreCount = 1;
	...
	//result = vkWaitForFences( LogicalDevice, 1, IN &renderFence, VK_FALSE, UINT64_MAX );	// wait for just 1, timeout
	//do
	//{
		//result = vkGetFenceStatus(LogicalDevice, IN renderFence);
	//} while (result == VK_NOT_READY);

	if (Verbose && NumRenders <= 3)		REPORT("vkWaitForFences");
...
//************************P4
	...
	if( UseRotate )
	{
		if (!Paused)
		{
			...
			Scene.uSceneOrient = glm::rotate(Scene.uSceneOrient, (float)glm::radians(360.f*Time / SECONDS_PER_CYCLE), glm::vec3(0., 1., 0.));
			Scene.uSceneOrient = glm::rotate(Scene.uSceneOrient, (float)glm::radians(90.f), glm::vec3(1., 0., 0.));
	...
	if( UseOrtho )
		...
		Scene.uProjection = glm::perspective( (float)FOV, (float)Width/(float)Height, 0.1f, 1000.f );
